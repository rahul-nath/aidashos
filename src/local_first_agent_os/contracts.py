...
class StageRoster(BaseModel):
    """Who is rostered for a stage and which Characters are summon-only.

    A Character on ``characters`` is preloaded into the pow-wow.
    A Character on ``summon_only`` is not preloaded — it can only be invoked
    via escalation (e.g., Staff during IMPLEMENTATION when a senior breaches
    a confidence threshold).
    """

    characters: list[CharacterName] = Field(default_factory=list)
    functional_roles: list[FunctionalRole] = Field(default_factory=list)
    summon_only: list[CharacterName] = Field(default_factory=list)
...
CANONICAL_SAGA_STAGES: list[StageDef] = [
    StageDef(
        stage=SagaStage.IDEA_INTAKE,
        goal="Normalize raw idea into a structured brief",
        roster=StageRoster(
            characters=[CharacterName.IDEATOR, CharacterName.REALIST],
            functional_roles=[FunctionalRole.SUMMARIZER],
        ),
        required_outputs=["normalized_idea_brief", "unresolved_questions", "risk_list"],
    ),
    StageDef(
        stage=SagaStage.GAWD_DOC,
        goal="Produce a complete GAWD doc with acceptance criteria and task graph",
        roster=StageRoster(
            characters=[CharacterName.STAFF_ENGINEER, CharacterName.PRODUCT_OWNER],
            functional_roles=[FunctionalRole.SECURITY_AGENT, FunctionalRole.QA_AGENT],
        ),
        required_outputs=["gawd_doc", "acceptance_criteria", "task_graph"],
    ),
    StageDef(
        stage=SagaStage.REQUIREMENT_DECOMPOSITION,
        goal="Break GAWD doc into an executable task graph",
        # Architect is a Mind that Staff summons here — not a roster entry.
        roster=StageRoster(characters=[CharacterName.STAFF_ENGINEER]),
        required_outputs=["task_graph_refined", "dependency_map"],
    ),
    StageDef(
        stage=SagaStage.IMPLEMENTATION,
        goal="Implement tasks per the task graph",
        roster=StageRoster(
            characters=[CharacterName.SENIOR_ENGINEER, CharacterName.JUNIOR_ENGINEER],
            functional_roles=[FunctionalRole.CODE_REVIEWER, FunctionalRole.TEST_RUNNER],
            summon_only=[CharacterName.STAFF_ENGINEER],
        ),
        required_outputs=[
            "pr_patch",
            "test_report",
    ...
    StageDef(
        stage=SagaStage.REVIEW_EVALUATION,
        goal="Evaluate all outputs against GAWD acceptance criteria",
        roster=StageRoster(
            characters=[CharacterName.STAFF_ENGINEER, CharacterName.REALIST],
            functional_roles=[FunctionalRole.QA_AGENT],
        ),
        required_outputs=[
            "evaluation_summary",
            "drift_report",
    ...
    StageDef(
        stage=SagaStage.USER_APPROVAL,
        goal="Present final artifacts to user for approval or deploy",
        roster=StageRoster(characters=[CharacterName.STAFF_ENGINEER]),
        required_outputs=["approval_decision", "deploy_summary"],
    ),
]


# ---------------------------------------------------------------------------
# Investigation saga (formerly "Researcher saga")
#
# A future sub-saga for BFS over data sources when the Researcher Mind alone
# is insufficient. Final stage gates on the Researcher Mind asking
# "what evidence do we actually have?" against the produced findings.
# Implementation lives in a follow-up; this stub locks the naming.
# ---------------------------------------------------------------------------

INVESTIGATION_SAGA_STAGES: list[StageDef] = []
