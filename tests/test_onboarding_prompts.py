...
def test_every_script_a_prompt_names_exists_in_both_dialects(prompts_document: dict) -> None:
    ...
    for stage in sorted(named_stages):
        for suffix in (".sh", ".ps1"):
            path = REPO_ROOT / "scripts" / "boot" / f"{stage}{suffix}"
            assert path.is_file(), f"prompt names {stage}{suffix}, which does not exist"
